...
class BaseMicro:
    ...
    def pre_sync(self):
        ...
        return self.serial.write(SYNC_BYTES)
...
class MicroManager:
    ...
    def reset(self):
        serials = tuple(
            GenericMicro(serial.Serial(usb.device, BAUDRATE, writeTimeout=0), self) for usb in comports()
        )

        serials = tuple(s for s in serials if s.pre_sync())
        time.sleep(1.)
